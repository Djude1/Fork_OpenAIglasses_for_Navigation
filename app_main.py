...
def load_navigation_models():
    ...
    if os.getenv("MODEL_SERVER_PORT"):
        try:
            ...
            print(
                f"[NAVIGATION] 共用模型伺服器模式：localhost:{os.getenv('MODEL_SERVER_PORT')}",
                flush=True,
            )
    ...
    try:
        ...
        if os.path.exists(seg_model_path):
            ...
            if torch.cuda.is_available():
                ...
                print(
                    f"[NAVIGATION] 盲道分割模型加载成功并放到GPU: {yolo_seg_model.device}"
                )
            else:
...
def _play_and_speak(audio_key: str):
    """ASR 音效：同時播放串流音效 + 推送 SPEAK 給 APP 本地播放（零延遲）"""
    play_audio_threadsafe(audio_key)
    _notify_speak(audio_key)
...
async def start_ai_with_text_custom(user_text: str):
    ...
    # 簡→繁統一：ASR 可能輸出簡體/繁簡混合，統一轉為繁體再匹配指令
    ...
    # 【修改】检查是否是过马路结束命令（必須在開始過馬路之前判斷）
    if (
        "过马路结束" in user_text
        or "结束过马路" in user_text
        or "過馬路結束" in user_text
        or "結束過馬路" in user_text
    ):
        if orchestrator:
    ...
    # 【修改】检查是否是过马路开始命令 - 使用orchestrator控制
    if (
        "开始过马路" in user_text
        or "帮我过马路" in user_text
        or "開始過馬路" in user_text
        or "幫我過馬路" in user_text
        or "我要過馬路" in user_text
        or "過馬路" in user_text and len(user_text) <= 4
    ):
        # 【新增】如果正在找物品，先停止
        if yolomedia_running:
            stop_yolomedia()
    ...
    # （過馬路結束已移到前面，避免被「過馬路」搶先匹配）
        return

    # 【修改】检查是否是红绿灯检测命令 - 实现与盲道导航互斥
    if (
        "检测红绿灯" in user_text
        or "看红绿灯" in user_text
        or "檢測紅綠燈" in user_text
        or "看紅綠燈" in user_text
    ):
        try:
            import trafficlight_detection
    ...
    if (
        ...
        or "停止檢測" in user_text
        or "停止紅綠燈" in user_text
    ):
        try:
            # 恢复到对话模式
            if orchestrator:
    ...
    if keyword_hits:
        print(
            f"[IDLE過濾] 關鍵字命中 {len(keyword_hits)} 個 {keyword_hits}，通過",
            flush=True,
        )
    ...
    async def _runner():
        ...
        try:
            async for piece in stream_chat(
                content_list, voice="Cherry", audio_format="wav"
            ):
                # 文本增量（仅 UI）
                if piece.text_delta:
...
async def verify_continuous(enabled: bool):
    ...
    return {
        "continuous": enabled,
        "message": f"持續監測已{status}，請看伺服器終端機輸出",
    }
...
async def bypass_wake(enabled: bool):
    ...
    return {
        "bypass_wake": enabled,
        "message": f"旁路模式已{status}，所有語音將直接送 AI 處理（不需喚醒詞）",
    }
...
async def verify_result():
    ...
    if len(_verify_test_buffer) == 0:
        return {
            "status": "no_data",
            "message": "尚未進行測試，請先呼叫 /api/test_speaker_verify",
        }
    try:
...
def api_debug_status():
    ...
    return {
        ...
        "recent_finals": recent_finals[-20:],
            ...
            if "text" in msg and msg["text"] is not None:
                ...
                if _verify_continuous:
                    ...
                    if len(_verify_continuous_buf) >= needed:
                        ...
                        try:
                            ...
                            rms = (
                                math.sqrt(sum(s * s for s in shorts) / len(shorts))
                                if shorts
                                else 0
                            )
                            ...
                            if rms < STANDBY_RMS_THRESH:
                                print(
                                    f"[VERIFY] 靜音（RMS={rms:.0f} < {STANDBY_RMS_THRESH}），跳過",
                                    flush=True,
                                )
                                asyncio.create_task(
                                    _speaker_event_push(
                                ...
                                passed, sim = speaker_verifier.verify_with_score(
                                    pcm_snap
                                )
                                if sim is None:
                                    print(
                                        "[VERIFY] 無法計算相似度（驗證未啟用或尚無聲紋）",
                                        flush=True,
                                    )
                                    asyncio.create_task(
                                        _speaker_event_push(
                ...
                if _verify_test_active:
                    ...
                    if now_mono < _verify_test_end_ts:
                        ...
                        print(
                            "[VERIFY_TEST] 收音完畢，等待查詢 /api/verify_result",
                            flush=True,
                        )
                ...
                if ws._audio_dbg_cnt % 100 == 1:
                    print(
                        f"[AUDIO-DBG] 已收 {ws._audio_dbg_cnt} 幀，streaming={streaming}",
                        flush=True,
                    )
...
async def ws_camera_esp(ws: WebSocket):
    ...
    try:
        await ws.send_text(
            "SET:QUALITY=15"
        )  # 畫質 15：縮小傳輸量，YOLO imgsz=320 精度已足夠
                ...
                while True:
                    try:
                        ...
                        if _fd:
                            frame_counter += 1
                            try:
                                sync_recorder.record_frame(_fd)
                            except Exception:
                                pass
                            try:
                                last_frames.append((time.time(), _fd))
                            except Exception:
                                pass
...
async def on_startup():
    ...
    try:
        ...
        print(
            f"[UDP] port {UDP_PORT} 無法綁定（{e}），IMU 資料將不可用，但服務繼續啟動",
            flush=True,
        )
    ...
    def _speak_push(text: str, duration_ms: int) -> None:
        msg = json.dumps(
            {"type": "speak", "text": text, "duration_ms": duration_ms},
            ensure_ascii=False,
        )
            ...
            try:
                ...
                if data == b"AIGLASS_DISCOVER":
                    ...
                    for dest in [addr, (subnet_bcast, addr[1])]:
                        try:
                            reply_sock = _socket.socket(
                                _socket.AF_INET, _socket.SOCK_DGRAM
                            )
                            reply_sock.setsockopt(
                            ...
                            pass
                    print(
                        f"[DISC] {addr[0]} 詢問 -> 回應 {my_ip}（unicast + bcast {subnet_bcast}）",
                        flush=True,
                    )
...
def _compress_image_b64(
    image_b64: str, max_width: int = 1280, quality: int = 80
) -> str:
    ...
    try:
        ...
        print(
            f"[DOC-READ] 圖片壓縮: {original_kb:.0f}KB → {new_kb:.0f}KB ({w}x{h} → {img.shape[1]}x{img.shape[0]})",
            flush=True,
        )
        return compressed
    except Exception as e:
...
async def api_read_document(req: ReadDocumentRequest):
    ...
    content_list = [
        ...
        {"type": "text", "text": "請完整擷取此圖片中的所有文字，保留原有排版與段落。"},
    ]
    text = await generate_text_async(
        content_list, system_prompt=system, max_tokens=8192
    )
...
async def api_explain_document(req: ExplainDocumentRequest):
    ...
    answer = await generate_text_async(
        content_list, system_prompt=system, max_tokens=2048
    )
