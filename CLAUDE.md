...
* **重點整理**：請每次都將討論出來的重點或是想法，放置到idea.md當中，來為了保證使用者不會忘記他當初的想法，或是需要修改的東西
* **Windows cmd 環境注意事項**：
  - 本專案在 Windows 環境開發，Shell 為 `cmd.exe`，不支援 bash 語法（`export`、`&&` 連接需注意）
  - 使用 `chcp 65001 > nul &` 作為命令前綴以確保 UTF-8 輸出
  - `jq` 未安裝，JSON 操作請用 `node -e` 替代
  - 避免使用 `cd <dir> && <command>` 模式，改用 `workdir` 參數
  - 檔案路徑含空格時必須用雙引號包裹
