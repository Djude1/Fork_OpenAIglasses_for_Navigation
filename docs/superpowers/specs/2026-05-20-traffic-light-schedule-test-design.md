...
> ## 交接提醒（接手成員與實作者必讀）
>
> 1. 本功能是**臨時可行性測試**，不是正式導航功能；隔離原則見第 2、3 節，拆除＝刪 2 檔＋還原路由 1 行。
> 2. **未完成的人工前置作業（無法自動化，必須現場做）**：每個要支援的路口、每個時段時制，需到現場親眼看到燈「剛轉綠」的瞬間記下手機系統時間，換成 Unix 秒填入對應 `SignalPlan.anchorEpochSec`。**沒填錨點的時段一律不播報**。
> 3. data.taipei 公開資料只給週期/時相秒數、**沒有即時相位**，所以步驟 2 不可省。資料為 2024-03-01 版（已逾一年），實作前請重新核對 `2026-05-20-traffic-light-data-台北商大.md` 的取得方法是否仍有效。
> 4. 實作完成後，於 `MD/現況快照.md` 更新此功能狀態與待測錨點清單，交接給下一位。
